...
Following Links in Python - Network Programming
...
Actual problem: 
Start at: http://py4e-data.dr-chuck.net/known_by_Niamh.html
...
# Specify link, tag position and iteration count
url = input("Enter link:")
position = int(input('Enter position:'))
repeat = int(input('Enter repetition count:'))

for i in range(repeat):
    html = urllib.request.urlopen(url, context=ctx).read()          # Open the connection to the URL
    soup = BeautifulSoup(html, 'html.parser')                       # HTML Beautiful Soup Parser
